...
- Constraints belong in the schema, not just the handler. Examples shipped in PR #49: `goals.crisis_origin NOT NULL`, the `goals_merge_consistency` CHECK, and the partial unique index on `undertaking_cycles` enforcing one-active-per-undertaking.
- `goal_amendments.cooldown_until` is managed by a `BEFORE INSERT OR UPDATE` trigger (`set_goal_amendment_cooldown`). The original plan called for a `GENERATED ALWAYS … STORED` column, but PG requires generation expressions to be IMMUTABLE and `timestamptz + interval` is STABLE. The trigger restores the original ironclad invariant: INSERT computes cooldown_until from proposed_at; UPDATE pins both `proposed_at` and `cooldown_until` back to their OLD values, so no client code path can move the cooldown forward or backward.
